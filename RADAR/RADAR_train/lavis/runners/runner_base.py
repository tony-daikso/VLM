...
class RunnerBase:
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if len(self.valid_splits) > 0:
                ...
                if not self.evaluate_only and (cur_epoch + 1) % save_interval == 0:
    ...
    def log_stats(self, stats, split_name, epoch=None):
        if isinstance(stats, dict):
            ...
            if epoch != None:
                epoch_state = {'epoch':str(epoch)}
                epoch_state.update(log_stats)
            else:
                epoch_state = log_stats
            
            with open(os.path.join(self.output_dir, "log.txt"), "a", encoding='utf-8') as f:
                # f.write(json.dumps(log_stats) + "\n")
                f.write(json.dumps(epoch_state, ensure_ascii=False) + "\n")
