...
class RadarPretrain(RadarBase, SharedQueueMixin, MomentumDistilationMixin):
    ...
    def __init__(
        ...
        # CT used 36 abdominal organs (TotalSegmentator); CheXmask only
        # provides 3 CXR anatomical masks (heart doubles as heart/mediastinum).
        # Must stay in sync with VisionBranch._organs() in vision_branch.py
        # and the region names used in RADAR/X-ray/phase2/region_rules.py.
        self.organs = ["left_lung", "right_lung", "heart"]
    ...
    def forward(self, samples):
        ...
        if cur_iters % 2 == 0 and self.radar_plus:
            ...
            with torch.no_grad():
                ...
                for i, pul_seg in enumerate(seg):
                    # 2D: pul_seg is (H, W) -- 4 edges of a rectangle, not the
                    # 6 faces of a cuboid the CT version checked
                    boundaries = [
                        pul_seg[0], pul_seg[-1],
                        pul_seg[:, 0], pul_seg[:, -1],
                    ]
                    ...
                    organ_intact_flags[i][intact_organ_ids - 1] = True    # [B, len(self.organs)]
        ...
        # compute seg loss (2D: last 2 dims are spatial (H,W), not 3)
