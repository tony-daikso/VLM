...
只練 head，之後解凍、用 discriminative LR 一起微調，見 STAGE2_PLAN.md 第 4 節）。
Best checkpoint / early stopping 都以 val macro AUROC 為準（只對 val 裡兩個 class 都有出現的
...
跑法：python3 U-VLM/X-ray/cxr/stage2/train.py [--config .../config.yaml] [--epochs N] [--resume last.pt]
"""
import argparse
import json
import os
...
def main():
    ...
    val_ds = Stage2Dataset(config, "validation", augment=False)
