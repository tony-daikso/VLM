"""
Encoder 沿用 CXR Stage 1 的 ResNet34Encoder 架構定義（跟 U-VLM/X-ray/cxr/stage1/model.py
逐層一致，獨立複製一份是為了讓每個 stage 資料夾自包含，跟 CT 版 stage2/model.py 的慣例一致 --
如果要改 encoder 架構，這份跟 stage1/model.py 要一起改，否則 best_encoder.pt 的 state_dict
會對不上）。這是完全獨立的一份程式碼，不 import CT 版的 model.py、不共用權重。

新增的 ClassificationHead 只用 encoder 最深層 f4（512 channel）做 GAP + 2 層 FC，輸出 26 維
sigmoid logits（不用 softmax，同一筆 study 可以同時有多個陽性 label_group），設計理由見
STAGE2_PLAN.md 第 3 節。
"""
import torch
