"""
共用 2D U-Net 風格 encoder（ResNet34, ImageNet 預訓練）+ 3 個獨立 decoder head。

Encoder 用 torchvision 的 resnet34，stem 改成 1-channel 輸入（RGB 預訓練權重取平均變成
1-channel，這是重用 ImageNet 權重的標準作法，見 _adapt_stem_to_grayscale）。輸出 5 個 stage
的 multi-scale feature maps {f0..f4}，對應 spatial stride {2,4,8,16,32} —— 這組 feature 之後
會被 Stage 3 的 multi-layer visual injection 使用，所以 stage 劃分現在就定下來、之後不能隨便改
（見 STAGE1_PLAN.md 第 4 節）。

三個 decoder（lesion/organ/vessel）結構相同（對稱 U-Net decoder + skip connection），只有
最後輸出層的 channel 數/activation 不同，彼此不share權重，只share encoder。
"""
import torch
...
class Stage1Model(nn.Module):
    def __init__(self, config):
        ...
        self.organ_head = SegmentationHead(
            self.encoder.out_channels, decoder_channels, out_channels=config["organ"]["num_classes"]
        )
        self.vessel_head = SegmentationHead(self.encoder.out_channels, decoder_channels, out_channels=2)

    def forward(self, x):
        output_size = x.shape[-2:]
        features = self.encoder(x)
        return {
            "lesion_logits": self.lesion_head(features, output_size),
            "organ_logits": self.organ_head(features, output_size),
            "vessel_logits": self.vessel_head(features, output_size),
        }

    def encoder_state_dict(self):
        """給 Stage 2/3 用：只存 encoder 權重，不帶三個 decoder。"""
