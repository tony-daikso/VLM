...
#     has_image: bool = False
# ) -> Dict:
#     # 用你剛剛在 conversation_lib 裡註冊的 qwen 模板
#     conv = conversation_lib.default_conversation.copy()
#     roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
#     # 2) 一次 tokenize（這是「最權威」的長度）
#     if has_image:
#         input_ids = torch.stack(
#             [tokenizer_image_token(p, tokenizer, return_tensors="pt") for p in conversations],
#             dim=0
#         )
#     else:
#         tokenized = tokenizer(
...
#     # 這是分隔 assistant 的片語
#     assistant_sep = conv.sep + conv.roles[1]   # "<|im_end|><|im_start|>assistant\n"

#     for conv_text, target in zip(conversations, targets):
#         total_len = int(target.ne(tokenizer.pad_token_id).sum())

#         # 按 qwen 格式切一輪一輪
#         # parts: [system..., user..., assistant..., user..., assistant..., ""]
#         parts = conv_text.split(conv.sep)
#         grouped = []
#         if len(parts) >= 3:
#             grouped.append(conv.sep.join(parts[:3]))  # system+user+assistant
#             for idx in range(3, len(parts), 2):
#                 grouped.append(conv.sep.join(parts[idx:idx+2]))
#         else:
#             grouped.append(conv_text)

#         cur = 0
#         for round_text in grouped:
#             if not round_text:
#                 continue

#             # round_text 裡要有 user + assistant
#             duo = round_text.split(assistant_sep)
#             if len(duo) != 2:
#                 # 沒 assistant 的整段不學
#                 if has_image:
#                     round_len = len(tokenizer_image_token(round_text, tokenizer))
#                 else:
#                     round_len = len(tokenizer(round_text).input_ids)
#                 target[cur:cur+round_len] = IGNORE_INDEX
#                 cur += round_len
#                 continue

#             user_part = duo[0] + assistant_sep

#             if has_image:
#                 round_len = len(tokenizer_image_token(round_text, tokenizer))
#                 user_len = len(tokenizer_image_token(user_part, tokenizer))
#             else:
#                 round_len = len(tokenizer(round_text).input_ids)
#                 user_len = len(tokenizer(user_part).input_ids)

#             # user 不學
#             target[cur:cur+user_len] = IGNORE_INDEX
#             # 後面 (assistant 那段) 保留原來的 label
#             cur += round_len

#         # 把 padding 後面的都關掉
#         target[cur:] = IGNORE_INDEX
#         if cur < total_len:
#             target[cur:total_len] = IGNORE_INDEX
...
def train(attn_implementation=None):
    ...
    #if training_args.load_in_4bit == True:
