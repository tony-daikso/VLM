...
from .loss import ClipLoss, CoCaLoss, SigLipLoss, TextDinoLoss
from .model import (
    CLIP,
    CustomTextCLIP,
    SigLIP,
...
def load_checkpoint(
    model: Union[CLIP, CustomTextCLIP, SigLIP, TextDino],
    checkpoint_path: str,
    strict: bool = True,
...
def create_model(
    ...
    **model_kwargs,
    ...
    if custom_text:
        ...
        elif model_kwargs.get('siglip', False):
            model = SigLIP(**model_cfg, cast_dtype=cast_dtype)
        elif model_kwargs.get('textdino', False) or "textdino_cfg" in model_cfg:
            model = TextDino(**model_cfg, cast_dtype=cast_dtype)
        else:
...
def create_loss(args):
    ...
    elif getattr(args, 'textdino', False):
        assert not args.horovod, "Horovod not currently supported for TextDino"
        return TextDinoLoss(
            rank=args.rank,
            world_size=args.world_size,
        )
...
def create_model_and_transforms(
    ...
    **model_kwargs,
    ...
    force_preprocess_cfg = [
        ...
        for mean, std in zip(image_mean, image_std)
    ]
    ...
    model = create_model(
        ...
        force_preprocess_cfg=force_preprocess_cfg[0],
    ...
    if multimodal_config is not None:
        with open(multimodal_config, "r") as f:
            mm_cfg = yaml.safe_load(f)
        # returns dictionary of preprocess configs
        preprocess_train, preprocess_val = {}, {}
        for modality, config in mm_cfg.items():
            ## build transformations for image preprocessing
            for cfg in force_preprocess_cfg:
                cfg["size"] = (
                    config["image_size"]
            ...
            pp_cfg = [PreprocessCfg(**cfg) for cfg in force_preprocess_cfg]

            preprocess_train[modality] = [
                image_transform_v2(
                    cfg,
                    is_train=True,
                    aug_cfg=aug_cfg,
            ...
            preprocess_val[modality] = [
                image_transform_v2(
                    cfg,
                    is_train=False,
                    # Added this since we would like to have access to `med_3d_aug` in val
                    aug_cfg=aug_cfg,
                )
                for cfg in pp_cfg
            ]
        ...
        ## build transformations for image preprocessing
        for cfg in force_preprocess_cfg:
            cfg["size"] = [model.visual.image_size, model.visual.image_size]
