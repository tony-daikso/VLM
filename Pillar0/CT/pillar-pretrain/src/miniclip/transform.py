...
class AugmentationCfg:
    ...
    # Augmentation for med3d
    med_3d_aug: bool = False
...
def image_transform(
    ...
    if aug_cfg.med_3d_aug:
        # 1 channel, designed for 3d volume (but the transform is performed per-slice)
        transform = [
            ToTensor(),
            # There is an if statement in Resize, if the shape matches the target shape, it will not resize.
            # ToTensor is performed first since we will use torch's resize (rather than PIL's) to resize the tensor. This allows using opencv (numpy) as the input of transform.
            Resize(image_size, interpolation=interpolation_mode),
            normalize,
        ]
        transform = Compose(transform)
        print(f"transform: {transform}, is_train: {is_train}")
        return transform
